...
float apprbf(const int *row_ptr, const int *col_ind, const int *weights, int **distance, 
	        const int nv, const int ne, int source, int iter_num, int min_edges, 
	        int max_edges, float percentage)
{
	int max_distance = INT_MAX;

	// Initialize GPU variables
	int *d_row_ptr, *d_col_ind, *d_weights, *d_nv, *d_max_distance;
	int *d_queue, *d_nextQueue, *d_nextSize, *d_iter, *d_min_edges, 
	    *d_max_edges, *d_subset_queue;
	float *d_percentage;
	int2* d_dist;
	...
	cudaCheck(cudaMalloc((void **)&d_subset_queue, (nv+1)*sizeof(int)));
	...
	*nextSize = 0;
	...
	curandGenerate(gen, d_random_ints, nv);
	...
	while((size > 0) && (round < iter_num+1)) { 

		size = percentage * size;

		cudaCheck(cudaMemcpy(d_size, &size, sizeof(int), cudaMemcpyHostToDevice));

		cudasubset_of_vertices<<<(size + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK>>>(d_size, d_percentage, d_queue, d_subset_queue, d_random_ints);

		std::swap(d_queue, d_subset_queue);

		cudarelaxAtomicMoreLessEdges<<<(size + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK>>>
