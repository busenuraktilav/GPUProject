...
float sbf(const int *row_ptr, const int *col_ind, const int *weights, int **distance, 
	     const int nv, const int ne, int source, int *iter_num)
{
	int max_distance = INT_MAX;

	// Initialize GPU variables
	int *d_row_ptr, *d_col_ind, *d_weights, *d_nv, *d_queue, *d_nextQueue, *d_nextSize, 
	    *d_iter, *d_max_distance;
	...
	*nextSize = 0;
	...
	while( size > 0 ) {
		...
		relaxAtomicOperationsBlock<<<(size + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK>>>
		                    (d_row_ptr, d_col_ind, d_weights, d_queue, d_nextQueue, d_size, d_nextSize, d_dist, d_iter); 
