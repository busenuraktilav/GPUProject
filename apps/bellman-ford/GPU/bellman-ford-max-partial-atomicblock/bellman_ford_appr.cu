...
float apprbf(const int *row_ptr, const int *col_ind, const int *weights, int **distance, 
	        const int nv, const int ne, int source, int max_edges, float percentage)
{
	int max_distance = INT_MAX;

	// Initialize GPU variables
	int *d_row_ptr, *d_col_ind, *d_weights, *d_nv, *d_max_distance;
	int *d_queue, *d_nextQueue, *d_nextSize, *d_iter, *d_max_edges, *d_subset_queue;
	float *d_percentage;
	int2* d_dist;
	...
	cudaCheck(cudaMalloc((void **)&d_max_edges, sizeof(int))); //for kernel cudarelaxAtomicMoreEdges, max edges
