...
float apprbf(const int *row_ptr, const int *col_ind, const int *weights, int **distance, 
	         const int nv, const int ne, int source, int iter_num, int max_edges, float percentage)
{
	...
	int *d_queue, *d_nextQueue, *d_nextSize, *d_iter, *d_subset_queue, *d_max_edges;
	float *d_percentage;
	int2* d_dist;
	...
	*nextSize = 0;
	...
	while((round < iter_num+1) ) {
		...
		cudarelaxAtomicLessEdgesBlock<<<(size + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK>>>
		                        (d_row_ptr, d_col_ind, d_weights, d_queue, d_nextQueue, d_size, d_nextSize, 
		                         d_dist, d_iter, d_max_edges);
