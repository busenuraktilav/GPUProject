...
float apprbf(const int *row_ptr, const int *col_ind, const int *weights, int **distance, 
	         const int nv, const int ne, int source, int iter_num, int min_edges, float percentage)
{
	...
	*nextSize = 0;
	...
	while( (size > 0) && (round < iter_num+1) ) {
		...
		cudarelaxAtomicMoreEdgesBlock<<<(size + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK>>>
		                        (d_row_ptr, d_col_ind, d_weights, d_queue, d_nextQueue, d_size, d_nextSize, 
		                         d_dist, d_iter, d_min_edges);
