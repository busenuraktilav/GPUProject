	        ...
	        const int nv, const int ne, int source, int iter_num)
{
	...
	*nextSize = 0;
	...
	while( (round < iter_num+1) ) {
		...
		relaxAtomicOperationsBlock<<<(size + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK>>>(d_row_ptr, d_col_ind, d_weights, d_queue, d_nextQueue, d_size, d_nextSize, d_dist, d_iter);
