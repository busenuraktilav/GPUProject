...
int main (int argc, char const *argv[])
{
	...
	int *row_ptr, *col_ind, *weights, *distance, *approximate_distance, max_weight, iter_num,
	     min_weight, nv, ne, max_degree = 0, neg_edge_count = 0, start = 0, min_edges = 0;
	...
	time = apprshybrid(row_ptr, col_ind, weights, &approximate_distance, nv, ne, start, max_edges, percentage, neg_edge_count);
	...
	if (write)
	{
		...
		write_performance_results(perf_results, time_results, error_results, nv, ne, max_degree, 
			                      signals, min_edges, max_degree, iter_num, percentage);
