	        ...
	        const int nv, const int ne, int source, int max_edges, float percentage, int neg_edge_count)
{
	...
	*distance = (int*)malloc(nv*sizeof(int)); 
    ...
    aint n = sizeof(arr) / sizeof(arr[0]);
	...
	while((count) != (k+2))
    {
		...
		while(!visitedEmptyRandom(visited, nv, arr, freq, n))
	    {
	        ...
	        CUDA_SSSP_KERNEL1_LessEdges_Block<<< (nv + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK >>>( d_row_ptr, d_col_ind, d_weights,
