...
float apprshybrid(const int *row_ptr, const int *col_ind, const int *weights, int **distance, 
	        const int nv, const int ne, int source, int max_edges, float percentage, int neg_edge_count)
{
	// Initialize GPU variables
	int *d_row_ptr, *d_col_ind, *d_weights, *d_distance, *d_visited, *d_nv, *d_ne,
	    *d_max_distance, *d_max_edges;
	...
	*distance = (int*)malloc(nv*sizeof(int)); 
	...
	while((count) != (k+2))
		...
		CUDA_SSSP_KERNEL1<<< (nv + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK>>>
		...
		while(!visitedEmptyRandom(visited, nv, arr, freq, n))
	        {
		        // execute the kernel
		        cudaCheck(cudaMemcpy( d_visited, visited, sizeof(int) * nv, cudaMemcpyHostToDevice ));
		        
		        CUDA_SSSP_KERNEL1_LessEdges<<< (nv + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK >>>( d_row_ptr, d_col_ind, d_weights,
