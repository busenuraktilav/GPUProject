...
float apprshybrid(const int *row_ptr, const int *col_ind, const int *weights, int **distance, 
	          const int nv, const int ne, int source, int iter_num, int max_edges, int neg_edge_count)
{
	...
	*distance = (int*)malloc(nv*sizeof(int)); 
	...
	while(r != (k+2))
    {
	    ...
	    {
	        // execute the kernel
	        CUDA_SSSP_KERNEL1_LessEdges_Block<<< (nv + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK >>> 
