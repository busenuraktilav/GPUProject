...
int main (int argc, char const *argv[])
{
	...
	int min_edges = atoi(argv[4]);
	float percentage = atof(argv[5]);

	int *row_ptr, *col_ind, *weights, *distance, *approximate_distance, max_weight, 
	     min_weight, nv, ne, max_degree = 0, neg_edge_count = 0, start = 0;
	...
	time = apprshybrid(row_ptr, col_ind, weights, &approximate_distance, nv, ne, start, iter_num, min_edges, percentage, neg_edge_count);
