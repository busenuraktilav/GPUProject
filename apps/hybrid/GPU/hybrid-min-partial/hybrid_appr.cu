...
float apprshybrid(const int *row_ptr, const int *col_ind, const int *weights, int **distance, 
	          const int nv, const int ne, int source, int min_edges, float percentage, int neg_edge_count)
{
	...
	*distance = (int*)malloc(nv*sizeof(int)); 
	...
	while(r != (k+2))
    {
	    ...
	    while(!visitedEmptyRandom(visited, nv, arr, freq, n))
	    {
	        ...
	        // execute the kernel
	        CUDA_SSSP_KERNEL1_MoreEdges<<< (nv + N_THREADS_PER_BLOCK - 1) / N_THREADS_PER_BLOCK, N_THREADS_PER_BLOCK >>> 
	                                       ( d_row_ptr, d_col_ind, d_weights, d_visited, d_distance, d_temp_distance,
	                                   		nv, ne, d_min_edges );
