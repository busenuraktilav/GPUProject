...
int read_graph_binary(FILE *bp, int **row_ptr, int **col_ind, int **weights, int *nv, int *ne, int *neg_edge_count)
{
	fread(nv, sizeof(int), 1, bp);

	fread(ne, sizeof(int), 1, bp);

	fread(neg_edge_count, sizeof(int), 1, bp);
...
int write_graph_binary(FILE *bp, int *row_ptr, int *col_ind, int *weights, int nv, int ne, int neg_edge_count)
{
    fwrite(&nv, sizeof(int), (size_t)1, bp);
    fwrite(&ne, sizeof(int), (size_t)1, bp);
    fwrite(&neg_edge_count, sizeof(int), (size_t)1, bp);
	              ...
	              int *min_weight)
{
    ...
    FILE *bp, *fp;
    sprintf(bfile, "%s.bin", graph_name);
    bp = fopen(bfile, "rb");

    if (bp != NULL) // read binary file 
    {
        printf("Reading binary file\n");
        read_graph_binary(bp, row_ptr, col_ind, weights, nv, ne, neg_edge_count);
    ...
    else // read from text
    {
        ...
        else
        {
            ...
            if (bp != NULL)
            {
                write_graph_binary(bp, *row_ptr, *col_ind, *weights, *nv, *ne, *neg_edge_count);
                fclose(bp);
            }
