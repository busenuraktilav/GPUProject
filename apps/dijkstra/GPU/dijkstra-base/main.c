...
int main (int argc, char const *argv[])
{
	...
	int *row_ptr, *col_ind, *weights, *distance, max_weight, min_weight, nv, ne, 
	     iter_num = 0, min_edges = 0, max_degree = 0, 
	     neg_edge_count = 0, start = 0;
	...
	time = sdj(row_ptr, col_ind, weights, &distance, nv, ne, start, &iter_num);	
	...
	if (write)
	{
		write_distance(distance_file, distance, iter_num, nv);
